...
app.http('TNTUsers',{methods:['GET','POST','PUT','OPTIONS'],authLevel:'anonymous',handler:async(req,ctx)=>{
 ...
 try{
  ...
  if(!roles.includes(b.role))return{status:400,headers:corsHeaders(),jsonBody:{error:'Ongeldige rol.'}};if(b.firstName!==undefined||b.lastName!==undefined||b.email!==undefined||b.isActive!==undefined){const ur=pool.request().input('UserId',sql.Int,+b.userId).input('FirstName',sql.NVarChar(100),String(b.firstName||'').trim()).input('LastName',sql.NVarChar(100),String(b.lastName||'').trim()).input('Email',sql.NVarChar(255),String(b.email||'').trim().toLowerCase()).input('IsActive',sql.Bit,b.isActive===false?0:1);await ur.query(`UPDATE dbo.Users SET FirstName=CASE WHEN @FirstName='' THEN FirstName ELSE @FirstName END,LastName=CASE WHEN @LastName='' THEN LastName ELSE @LastName END,Email=CASE WHEN @Email='' THEN Email ELSE @Email END,IsActive=@IsActive WHERE UserId=@UserId`);}
  ...
  const tx=new sql.Transaction(pool);await tx.begin();try{await new sql.Request(tx).input('UserId',sql.Int,+b.userId).input('Role',sql.NVarChar(30),b.role).query(`UPDATE dbo.Users SET Role=@Role WHERE UserId=@UserId`);await setTeams(tx,+b.userId,b.role,teamIds);await tx.commit();return{status:200,headers:corsHeaders(),jsonBody:{success:true}}}catch(e){await tx.rollback();throw e}
